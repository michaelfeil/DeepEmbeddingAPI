...
if __name__ == "__main__":
    ...
    parser.add_argument("--recordtime", "-r", help="Time to store embeddings as person name from camera", type=int, default=6)
    ...
    while True:
        ...
        if (time.time() - start_time) > args.recordtime:
            break
        path = os.path.join(PATH_ENCODED_DB, args.name, f"{args.name}_record_{time.time():.2f}.png")
